            ...
            case "Submit":
                if((cBmetodepembayaran.getItemAt(cBmetodepembayaran.getSelectedIndex()).equals("Tunai")) || (CustomerManager.getInstance().getCustomer().getSaldoUp() >= totalharga)){
                    ...
                    for(int i = 0; i < listDriver.size(); i++){
                        if(listDriver.get(i).getStatus().equals("AVAILABLE") && listDriver.get(i).getJeniskendaraan().equals(PesananOjekManager.getInstance().getPesananojek().getJenisKendaraan())){
                            cek = true;
